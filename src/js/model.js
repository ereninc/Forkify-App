...
const createRecipeObject = function (data) {
  ...
  return {
    ...
    ...(recipe.key && { key: recipe.key }), //If recipe.key exists then add it to the object, good way to add optional (conditionally) properties
  };
};
...
export const uploadRecipe = async function (newRecipe) {
  try {
    ...
    const data = await helpers.sendJSON(`${API_URL}?key=${KEY}`, recipe);
    state.recipe = createRecipeObject(data);
